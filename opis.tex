...
V nadaljevanju bova definirala funkcijo \texttt{alpha\_od(G)}, ki za dan graf $G$ izračuna liho
neodvisno število $\alpha_{od}(G)$. Funkcija bo temeljila na pregledu vseh podmnožic množice vozlišč
grafa $G$ z uporabo razreda \texttt{Subsets(G.vertices())}. Za vsako kandidatno množico $S$ bova
preverila, ali je neodvisna z uporabo metode \texttt{G.has\_edge(u,v)} ter ali izpolnjuje pogoje
lihe neodvisnosti z uporabo metode \texttt{G.neighbors(v)}. Ker je takšen pristop eksponenten,
bo uporaben predvsem za grafe z manjšim številom vozlišč.
...
\]
Rezultate bova po potrebi shranjevala ter primerjala z uporabo metode \texttt{G.is\_isomorphic(H)},
da se izogneva podvajanju izomorfnih grafov.
